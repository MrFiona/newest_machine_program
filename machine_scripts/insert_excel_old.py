...
class InsertDataIntoExcel(object):
    def __init__(self, session, query, silver_url_list, section_Silver_url_list, link_WW_week_string, verify_flag=False, purl_bak_string=None, cache=None,
        ...
        #todo 在自定义模式下并且选择了candidate周执行,会更新save_miss_insert_bkc_string的值
        ...
        #todo OSS2与OSS1周位置分隔 目前只适配一个项目
        if self.purl_bak_string == 'Bakerville':
            oss2_oss1_separate_index = self.Silver_url_list.index('https://dcg-oss.intel.com/ossreport/auto/Bakerville/Silver/2017%20WW50/16051_Silver.html')
            #todo 属于oss2的周url列表
            self.oss2_silver_url_list = self.Silver_url_list[:oss2_oss1_separate_index+1]
        self.logger.print_message('self.oss2_silver_url_list:\t%s' % self.oss2_silver_url_list, self.__file_name)
    ...
    def insert_NewSi_data(self):
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if j == 0:
                    Silver_BkC_string, date_string, effective_url_list, header_list, cell_data_list = obj.get_new_sightings_data(key_word, self.verify_flag)
                    ...
                    Silver_BkC_string, date_string, effective_url_list, header_list, cell_data_list = obj.get_new_sightings_data(key_word, True)
    ...
    def insert_ExistingSi_data(self):
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if j == 0:
                    Silver_BkC_string, date_string, url_list, header_list, cell_data_list = obj.get_existing_sighting_data(key_word, self.verify_flag)
                    if self.keep_continuous != 'YES':
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                    elif self.keep_continuous == 'YES' and self.equal_silver_list_flag:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                else:
                    Silver_BkC_string, date_string, url_list, header_list, cell_data_list = obj.get_existing_sighting_data(key_word, True)
                    if self.keep_continuous == 'YES' and j == self.actual_newest_week_position:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if self.purl_bak_string == 'Purley-Crystal-Ridge':
                    ...
                    obj = extract_data_old.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)
                #todo 最新的一周在验证标志未开启的情况下取Silver数据,否则取BKC数据
                if j == 0:
                    if self.purl_bak_string == 'Purley-FPGA' or self.purl_bak_string == 'NFVi' or self.purl_bak_string == 'Purley-Crystal-Ridge':
                        Silver_BkC_string, object_string_list, date_string = obj.get_rework_data(key_word, self.verify_flag)
                    elif self.purl_bak_string == 'Bakerville' and self.Silver_url_list[j] in self.oss2_silver_url_list:
                        Silver_BkC_string, object_string_list, date_string = obj.get_rework_data(key_word, self.verify_flag)
                    else:
                        Silver_BkC_string, object_string_list, date_string = obj.get_bak_rework_data(key_word, self.verify_flag)
                    ...
                    if self.purl_bak_string == 'Purley-FPGA' or self.purl_bak_string == 'NFVi' or self.purl_bak_string == 'Purley-Crystal-Ridge':
                        Silver_BkC_string, object_string_list, date_string = obj.get_rework_data(key_word, True)
                    elif self.purl_bak_string == 'Bakerville' and self.Silver_url_list[j] in self.oss2_silver_url_list:
                        Silver_BkC_string, object_string_list, date_string = obj.get_rework_data(key_word, True)
                    else:
                        Silver_BkC_string, object_string_list, date_string = obj.get_bak_rework_data(key_word, True)
            ...
            if j == 0:
                if self.purl_bak_string == 'Purley-FPGA' or self.purl_bak_string == 'NFVi':
                    if self.purl_bak_string == 'Purley-FPGA' and FPGA_border_position >= j:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_lastest_FPGA_hw_data(key_word, self.verify_flag)
                    else:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_hw_data(key_word, self.verify_flag)
                elif self.purl_bak_string == 'Purley-Crystal-Ridge':
                    Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_hw_data(key_word, self.verify_flag)
                #todo Bakerville项目
                else:
                    if DE_border_position >= j and self.purl_bak_string == 'Bakerville' and self.Silver_url_list[j] not in self.oss2_silver_url_list:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_lastest_bak_hw_data(key_word, self.verify_flag)
                        if Silver_BkC_string == 'Error' and date_string == 'Error' and not row_coordinates_list and not column_coordinates_list and not cell_data_list:
                            Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_bak_hw_data(key_word, self.verify_flag)
                    #todo oss2模板
                    elif self.purl_bak_string == 'Bakerville' and self.Silver_url_list[j] in self.oss2_silver_url_list:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_hw_data(key_word, self.verify_flag)
                    else:
                ...
                if self.purl_bak_string == 'Purley-FPGA' or self.purl_bak_string == 'NFVi':
                    if self.purl_bak_string == 'Purley-FPGA' and FPGA_border_position >= j:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_lastest_FPGA_hw_data(key_word, True)
                    else:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_hw_data(key_word, True)
                elif self.purl_bak_string == 'Purley-Crystal-Ridge':
                    Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_hw_data(key_word, True)
                #TODO DE
                else:
                    if DE_border_position >= j and self.purl_bak_string == 'Bakerville' and self.Silver_url_list[j] not in self.oss2_silver_url_list:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_lastest_bak_hw_data(key_word, True)
                        if Silver_BkC_string == 'Error' and date_string == 'Error' and not row_coordinates_list and not column_coordinates_list and not cell_data_list:
                            Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_bak_hw_data(key_word, True)
                    #todo oss2模板
                    elif self.purl_bak_string == 'Bakerville' and self.Silver_url_list[j] in self.oss2_silver_url_list:
                        Silver_BkC_string, date_string, row_coordinates_list, column_coordinates_list, cell_data_list = obj.get_hw_data(key_word, True)
                    else:
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if self.purl_bak_string == 'Purley-Crystal-Ridge':
                    ...
                    obj = extract_data.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)
                    key_word = 'SWConfiguration'
                else:
                    obj = extract_data_old.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)

                if j == 0:
                    if 'Purley-FPGA/Silver/2017%20WW31' in self.Silver_url_list[j]:
                        Silver_BkC_string, header_length, date_string, url_list, header_list, cell_data_list = obj.get_sw_data_1(key_word, self.verify_flag)
                    else:
                        Silver_BkC_string, header_length, date_string, url_list, header_list, cell_data_list = obj.get_sw_data(key_word, self.verify_flag)
                    if self.keep_continuous != 'YES':
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                    elif self.keep_continuous == 'YES' and self.equal_silver_list_flag:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                    ...
                    if 'Purley-FPGA/Silver/2017%20WW31' in self.Silver_url_list[j]:
                        Silver_BkC_string, header_length, date_string, url_list, header_list, cell_data_list = obj.get_sw_data_1(key_word, True)
                    else:
                        Silver_BkC_string, header_length, date_string, url_list, header_list, cell_data_list = obj.get_sw_data(key_word, True)
                    if self.keep_continuous == 'YES' and j == self.actual_newest_week_position:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if self.purl_bak_string == 'Purley-Crystal-Ridge':
                    ...
                    obj = extract_data.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)
                    key_word = 'IFWIConfiguration'
                else:
                    obj = extract_data_old.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)
                if j == 0:
                    Silver_BkC_string, date_string, header_list, cell_data_list = obj.get_ifwi_data(key_word, self.verify_flag)
                    if self.keep_continuous != 'YES':
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                    elif self.keep_continuous == 'YES' and self.equal_silver_list_flag:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                else:
                    Silver_BkC_string, date_string, header_list, cell_data_list = obj.get_ifwi_data(key_word, True)
                    if self.keep_continuous == 'YES' and j == self.actual_newest_week_position:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                #todo Set up some formats to use.
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if self.purl_bak_string == 'Purley-Crystal-Ridge':
                    ...
                    obj = extract_data.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)
                    key_word = 'PlatformIntegrationValidationResult'
                else:
                    obj = extract_data_old.GetAnalysisData(self.Silver_url_list[j], get_info_not_save_flag=False, cache=self.cache, insert_flag=True, logger=self.logger, purl_bak_string=self.purl_bak_string)

                if j == 0:
                    Silver_BkC_string, date_string, url_list, header_list, cell_data_list = obj.get_platform_data(key_word, self.verify_flag)
                    if self.keep_continuous != 'YES':
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                    elif self.keep_continuous == 'YES' and self.equal_silver_list_flag:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
                else:
                    Silver_BkC_string, date_string, url_list, header_list, cell_data_list = obj.get_platform_data(key_word, True)
                    if self.keep_continuous == 'YES' and j == self.actual_newest_week_position:
                        self.newest_week_type_string_list.append(Silver_BkC_string)
    ...
    def insert_CaseResult(self):
        ...
        for j in range(len(self.Silver_url_list)):
            ...
            try:
                ...
                if j == 0:
                    date_string, Silver_Gold_BKC_string, tip_string, header_list, cell_data_list = obj.get_caseresult_data(key_word, self.verify_flag)
                    if self.keep_continuous != 'YES':
                        self.newest_week_type_string_list.append(Silver_Gold_BKC_string)
                    elif self.keep_continuous == 'YES' and self.equal_silver_list_flag:
                        self.newest_week_type_string_list.append(Silver_Gold_BKC_string)
                else:
                    date_string, Silver_Gold_BKC_string, tip_string, header_list, cell_data_list = obj.get_caseresult_data(key_word, True)
                    if self.keep_continuous == 'YES' and j == self.actual_newest_week_position:
                        self.newest_week_type_string_list.append(Silver_Gold_BKC_string)
    ...
    def _insert_test_case_info_into_excel(self):
        #todo 获取test-plan中项目所有的test-case
        self.logger.print_message(u'开始获取项目[%s]在HPQC中的test-case信息' % self.purl_bak_string, self.__file_name)
        test_case = GetHPQCTestPlanCase(self.logger, self.session, self.query, self.purl_bak_string)
        ...
        for test_case in new_data:
            ...
            test_case_info = [test_case[1][u'_domain'], u'BKC', test_case[1][u'_test_name'], start_flag, 0]
            self.worksheet_caseresult.write_row(init_row, 0, test_case_info, self.header_format)
            init_row += 1

        #todo 每周固定位置插入所有的最新的test-case
        for j in range(len(self.Silver_url_list)):
            for nu in xrange(len(new_data)):
